...
static int kernel_read_file_helper(struct file *file, void **buf, loff_t *size,
				   ...
				   bool partial)
{
	...
	if (partial && (i_size > (pos + max_size)))
	...
	if (!*buf)
		*buf = allocated = vmalloc(alloc_size);
	if (!*buf) {
		ret = -ENOMEM;
		goto out;
	}
...
int kernel_pread_file(struct file *file, void **buf, loff_t *size,
		      ...
		      enum kernel_read_file_id id)
{
	return kernel_read_file_helper(file, buf, size, max_size, pos,
				       id, true);
}
...
{
	return kernel_read_file_helper(file, buf, size, max_size, 0,
				       id, false);
}
